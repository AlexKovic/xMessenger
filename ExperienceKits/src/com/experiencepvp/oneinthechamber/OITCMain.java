...
public class OITCMain {
	static SettingsManager settings = SettingsManager.getInstance();
	
	public static HashMap<String, OITCMap> inqueue = new HashMap<String, OITCMap>();
	
	public static boolean open;
	
	public static OITCMap runningmap = null;
	
	public static int players;
	
	public static HashMap<String, OITCMap> maps = new HashMap<String, OITCMap>();
	...
	public static void startTourney(final OITCMap map){
		EventsHandler.running = true;
		EventsHandler.runningevent = "oitc";
		OITCMain.open = true;
		OITCMain.runningmap = map;
		map.startTimer();
		new BukkitRunnable() {
			@SuppressWarnings("deprecation")
			public void run() {
				for(String s : OITCMain.inqueue.keySet()){
					if(Bukkit.getServer().getPlayer(s) != null){
						OITCMain.maps.put(s, OITCMain.inqueue.get(s));
					}
				}

				OITCMain.inqueue.clear();
				
				if(OITCMain.maps.size() < 2){
				...
				for (Player on : Bukkit.getOnlinePlayers()) {
					if(OITCMain.maps.containsKey(on.getName())){
						if (OITCMain.maps.get(on.getName()) == map) {
							on.teleport(map.getSpawnpoint());
							OITCMain.invincible.add(on.getName());
		...
		new BukkitRunnable() {
			...
			public void run() {
				for (Player on : Bukkit.getOnlinePlayers()) {
					if (OITCMain.invincible.contains(on.getName()) && OITCMain.maps.get(on.getName()) == map) {
						OITCMain.invincible.remove(on.getName());
	...
	public static void joinTourney(Player p){
		...
		OITCMain.inqueue.put(p.getName(), runningmap);
		OITCMain.players = OITCMain.players + 1;
		p.sendMessage("§aYou have successfully joined the §lqueue! §7(" + OITCMain.players + "/" + runningmap.getMaxPlayers() + ")");
		...
		for(Player on : Bukkit.getServer().getOnlinePlayers()){
			if(OITCMain.inqueue.containsKey(on.getName())){
				if(OITCMain.inqueue.get(on.getName()) == runningmap){
					on.sendMessage("§a§l" + p.getName() + "§f§a has joined the queue! §7(" + OITCMain.players + "/" + runningmap.getMaxPlayers() + ")"); 
				}
			}
		}
	}
