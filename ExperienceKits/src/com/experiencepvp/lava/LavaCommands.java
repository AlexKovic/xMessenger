					...
					if (EventsHandler.canHost) {
							if (EventsHandler.running == false) {
								if (TourneyMain.isInt(args[1])) {
									...
									if (LavaMap.getMapById(map) != null) {
										LavaMap m = LavaMap.getMapById(map);
										LavaMain.startTourney(m);
									} else {
...
//								if (TourneyMain.isInt(args[1])) { 
//									int map = Integer.parseInt(args[1]);
//									LavaUtil.setLocation(map, p.getLocation());
...
//							p.sendMessage("§cWrong syntax; use §l/lava setmaxplayers <map> <amount>");
//						}
//						if (args.length > 2) {
...
//								if (TourneyMain.isInt(args[1])) {
//									int map = Integer.parseInt(args[1]);
//										LavaUtil.setMaxPlayers(map, max);
