...
public class ComboCommands implements CommandExecutor {
					...
					if (EventsHandler.canHost) {
							if (EventsHandler.running == false) {
								if (TourneyMain.isInt(args[1])) {
									...
									if (ComboMap.getMapById(map) != null) {
										ComboMap m = ComboMap.getMapById(map);
										ComboMain.startTourney(m);
									} else {
...
//								if (TourneyMain.isInt(args[1])) {
//									int map = Integer.parseInt(args[1]);
//									ComboUtil.setLocation(map, p.getLocation());
...
//							p.sendMessage("§cWrong syntax; use §l/combo setmaxplayers <map> <amount>");
//						}
//						if (args.length > 2) {
...
//								if (TourneyMain.isInt(args[1])) {
//									int map = Integer.parseInt(args[1]);
//										ComboUtil.setMaxPlayers(map, max);
