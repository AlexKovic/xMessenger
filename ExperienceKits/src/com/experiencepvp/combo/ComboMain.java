...
public class ComboMain {
	...
	public static HashMap<String, ComboMap> inqueue = new HashMap<String, ComboMap>();
	
	public static boolean playing;
	
	public static boolean open;
	
	public static ComboMap runningmap = null;
	
	public static int players;
	
	public static HashMap<String, ComboMap> maps = new HashMap<String, ComboMap>();
	...
	public static void setupKits(Player p) {
		...
		p.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 9999999, 250));
	}
	...
	// if(SettingsManager.getInstance().getData().contains("stats." +
	// oPlayer.getUniqueId() + ".wins")){
	// if(SettingsManager.getInstance().getData().getInt("stats." +
	// oPlayer.getUniqueId() + ".wins") >= wins1){
	// op1 = oPlayer;
	// w1 = oPlayer.getName();
	// wins1 = SettingsManager.getInstance().getData().getInt("stats." +
	// oPlayer.getUniqueId() + ".wins");
	// }
	// }
	// }
	...
	// oPlayer.getUniqueId() + ".wins")){
	// if(SettingsManager.getInstance().getData().getInt("stats." +
	// oPlayer.getUniqueId() + ".wins") >= wins2){
	// op2 = oPlayer;
	// w2 = oPlayer.getName();
	// wins2 = SettingsManager.getInstance().getData().getInt("stats." +
	// oPlayer.getUniqueId() + ".wins");
	// }
	// }
	// }
	...
	// oPlayer.getUniqueId() + ".wins")){
	// if(SettingsManager.getInstance().getData().getInt("stats." +
	// oPlayer.getUniqueId() + ".wins") >= wins3){
	// op3 = oPlayer;
	// w3 = oPlayer.getName();
	// wins3 = SettingsManager.getInstance().getData().getInt("stats." +
	// oPlayer.getUniqueId() + ".wins");
	// }
	// }
	// }
	...
	// oplayers.remove(op3);
	//
	// Location loc1 = new Location(Bukkit.getServer().getWorlds().get(0),
	// -5153, 10, -5242);
	// Location loc2 = new Location(Bukkit.getServer().getWorlds().get(0),
	// -5152, 10, -5242);
	// Location loc3 = new Location(Bukkit.getServer().getWorlds().get(0),
	// -5151, 10, -5242);
	//
	...
	// skull3.update();
	//
	// Location sl1 = new Location(Bukkit.getServer().getWorlds().get(0), -5153,
	// 9, -5241);
	// Location sl2 = new Location(Bukkit.getServer().getWorlds().get(0), -5152,
	// 9, -5241);
	// Location sl3 = new Location(Bukkit.getServer().getWorlds().get(0), -5151,
	// 9, -5241);
	//
	...
	// s1.setLine(1, w1);
	// s1.setLine(2, "Wins: " +
	// SettingsManager.getInstance().getData().getInt("stats." +
	// op1.getUniqueId() + ".wins"));
	// s1.setLine(3, "================");
	// org.bukkit.material.Sign matSign = new
	// org.bukkit.material.Sign(Material.WALL_SIGN);
	// matSign.setFacingDirection(BlockFace.SOUTH);
	// s1.setData(matSign);
	// s1.update();
	// }else{
	// if(Bukkit.getServer().getPlayer("alexkovic") != null){
	// Bukkit.getServer().getPlayer("alexkovic").sendMessage("" +
	// sl1.getBlock().getState().toString());
	// }
	// }
	//
	...
	public static void startTourney(final ComboMap map){
		playing = true;
		open = true;
		runningmap = map;
		EventsHandler.runningevent = "combo";
		EventsHandler.running = true;
		ComboUtil.startTimer(map);
		new BukkitRunnable() {
			@SuppressWarnings("deprecation")
			public void run() {
	...
	public static void joinTourney(Player p){
		...
		inqueue.put(p.getName(), runningmap);
		players = players + 1;
		p.sendMessage("§aYou have successfully joined the §lqueue! §7(" + players + "/" + runningmap.getMaxPlayers() + ")");
		p.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "Events are currently in Beta! Please report any bugs to the staff members!");
		for(Player on : Bukkit.getServer().getOnlinePlayers()){
			if(inqueue.containsKey(on.getName())){
				if(inqueue.get(on.getName()) == runningmap){
					if (on != p) {
						on.sendMessage("§a§l" + p.getName() + "§f§a has joined the queue! §7(" + players + "/" + runningmap.getMaxPlayers() + ")");
					}
				}
			}
		}
